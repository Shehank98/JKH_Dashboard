...
* **Ad type** filter: All (default), Commercials or Sponsorships. Sponsorships are the Advt_Theme items `-BB`, `Com Break`, `DJ`, `-Extro`, `-Intro`, `-LLogo`, `Next Card`, `Tag`, `Time Check`, `-Tr` (exact name, or ending in a dash marker such as `Summer Promo -BB`); everything else is a commercial. The choice applies to every card and pop-up, and Commercials + Sponsorships always add up to All. In All and Commercials, sponsorship items are never picked as a lead campaign; in Sponsorships the top item is shown.
