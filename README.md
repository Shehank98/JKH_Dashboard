...
* **Share of Spend Comparison**: my advertiser, each competitor and all other advertisers for the selected period, with SOS, spend, spots and rank. A **Period | By month** toggle switches to a grid of each month's SOS % (shaded, darker = bigger share). Click a row or cell for details (rows open on a Months tab with share of category per month).
